        ...
        public static void FileTransform(FileData input, FileData output, DataProtectionConfig config, TokenCredential credential)
        {
            ...
            switch (output.FileType)
            {
                ...
                case FileType.parquet:
                    ...
                    var settings = ColumnSettings.Load(config, header, new AzureKeyVaultKeyStoreProvider(credential), output.IsEncrypted);
                    writer = new ParquetFileWriter(File.OpenWrite(output.FilePath), settings);
                    break;
            }
